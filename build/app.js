...
app.set("views", _path["default"].join(__dirname, "views")); // app.use("/uploads", express.static("uploads")); // /uploads이라는 route로 접근하면 view나 controller를 확인하는 것이 아니라 uploads 디렉토리로 접근

app.use("/static", _express["default"]["static"](_path["default"].join(__dirname, "static"))); // /static이라는 route 접근하면 view나 controller를 확인하는 것이 아니라 static 디렉토리로 접근

app.use((0, _cookieParser["default"])()); // Cookie Control

app.use(_bodyParser["default"].json()); // 서버가 body로부터 정보를 받아올 수 있음: ex) json 데이터를 받아와서 이해

app.use(_bodyParser["default"].urlencoded({
  extended: true
})); // 서버가 body로부터 정보를 받아올 수 있음: ex) Form 데이터를 서버에 의해 받아 활용가능

app.use((0, _morgan["default"])("dev")); // logger, 무슨 일이 어디서 일어났는지 기록
...
app.use(_passport["default"].session()); //쿠키에서 해당되는 유저를 찾아 req.user 생성, session을 저장

app.use(_middlewares.localsMiddleware); // local 변수를 global 변수로 사용
